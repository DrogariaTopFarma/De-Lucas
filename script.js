...
const database = [
    ...
    { id: 18, cat: 'Mamãe e Bebê', nome: 'Sabonete Liquido granado', price: 9.90, img: imgDefault, desc: 'Essencial para a limpeza onde a escova não alcança.' },
...
function sendWhatsApp() {
    ...
    const num = document.getElementById('numero_end').value.trim(); // Novo
    const ref = document.getElementById('referencia_end').value.trim(); // Novo
    const pag = document.getElementById('pagamento').value;
    
    // Validações básicas
    if (nome.split(" ").length < 2) return alert("Preencha o nome completo!");
    if (end.length < 5) return alert("Preencha o endereço!");
    if (num === "") return alert("Preencha o número do endereço!"); // Validação do número
