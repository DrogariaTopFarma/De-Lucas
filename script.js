...
// 1. INICIALIZAÇÃO E MÁSCARA DE MOEDA (O QUE FALTAVA)
function init() {
    ...
    // AQUI ESTÁ A MÁSCARA DA MOEDA PARA O TROCO
    ...
    if(trocoInput) {
        trocoInput.addEventListener('input', function(e) {
            ...
            e.target.value = "R$ " + value; // Adiciona o R$ dinamicamente
        });
    }
}

// 2. BUSCA COM MENSAGEM PERSONALIZADA
...
// 3. MODAL (INICIA COM QTD DO CARRINHO OU 0)
...
function changeModalQty(val) { 
    modalQtyCount += val; 
    if(modalQtyCount < 0) modalQtyCount = 0; 
    document.getElementById('modal-qty').innerText = modalQtyCount; 
}
...
// 6. FINALIZAR (WHATSAPP COM PAGAMENTO E TROCO)
