...
function sendWhatsApp() {
    ...
    const referencia = document.getElementById('referencia_end').value.trim();
    const pagamento = document.getElementById('pagamento').value;
    
    // Lógica do troco
    const precisaTroco = document.querySelector('input[name="troco_op"]:checked')?.value;
    ...
    if (!nome || !endereco || !numero || !referencia) {
        alert("⚠️ Por favor, preencha todos os campos de entrega (Nome, Endereço, Número e Referência).");
        return; // Para o código aqui e não envia pro Zap
    }

    // 3. Validação específica do Troco (se escolher dinheiro e marcar "sim")
    if (pagamento === "Dinheiro" && precisaTroco === "sim" && !valorTroco) {
        alert("⚠️ Por favor, informe para quanto você precisa de troco.");
        return;
    }

    // Se passou por tudo, o código continua aqui embaixo para montar a mensagem...
    // (Mantenha aqui o restante do seu código que gera a URL do WhatsApp)
    alert("Pedido validado! Abrindo WhatsApp...");
}
